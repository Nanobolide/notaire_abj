...
export async function PATCH(req, { params }) {
  try {
    ...
    const ligne = await withTenant(s.etudeId, async (c) => {
      ...
      if (action === "desactiver")   sql = `UPDATE utilisateurs SET actif = false WHERE id = $1 AND role <> 'admin_etude'`;
      else if (action === "reactiver") sql = `UPDATE utilisateurs SET actif = true WHERE id = $1`;
        ...
        vals = [hash, params.id];
        sql = `UPDATE utilisateurs SET hash_mot_de_passe = $1, doit_changer_mdp = true,
               verrouille_jusqua = NULL, echecs_connexion = 0 WHERE id = $2`;
      } else if (action === "definir_mdp") {
        // C3 — Le Notaire CHOISIT un nouveau mot de passe (au lieu d'en générer un aléatoire).
        // Le compte n'est PAS forcé de le changer ensuite : c'est un choix délibéré, pas un provisoire.
        if (!motDePasseProvisoire || motDePasseProvisoire.length < 8) {
          const e = new Error("Le nouveau mot de passe doit faire au moins 8 caractères."); e.status = 400; throw e;
        }
        const hash = await bcrypt.hash(motDePasseProvisoire, 10);
        vals = [hash, params.id];
        sql = `UPDATE utilisateurs SET hash_mot_de_passe = $1, doit_changer_mdp = false,
               verrouille_jusqua = NULL, echecs_connexion = 0 WHERE id = $2`;
        ...
        vals = [nom_affiche || null, nom_complet || null, fonction || null, niveau_acces || "standard", role, params.id];
...
export async function DELETE(req, { params }) {
  try {
    ...
    await withTenant(s.etudeId, async (c) => {
      const r = await c.query(`DELETE FROM utilisateurs WHERE id = $1 RETURNING identifiant`, [params.id]);
