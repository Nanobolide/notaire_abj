...
export async function GET(req, { params }) {
  try {
    ...
    const prevision = saisitPrevision(s);   // le Clerc rédacteur voit sa prévision
    const notes = true;                     // P2.5 — le Comptable voit comme un collaborateur
    ...
    await withTenant(s.etudeId, async (c) => {
      if (registre === "actes") {
        ...
        const cols = [
          ...
          // C22 — l'export dit la MÊME vérité que les écrans : la ventilation fait foi.
          ...
          // La prévision du clerc, affichée à part et jamais confondue avec le total facturé.
          ...(prevision && !admin ? [["Total des frais (FCFA)", "honoraires_totaux", 17],
                                     ["Réglé (FCFA)", "montant_regle", 13]] : []),
          ...(admin ? [["Total des frais (FCFA)", "honoraires_totaux", 17]] : []),
          // C23 — notes internes : jamais pour le Comptable.
        ...
        for (const a of rows) {
          ...
          if (admin) {
            ...
            const total = n(a.honoraires_totaux);   // P2.1 — ce que paie le client
