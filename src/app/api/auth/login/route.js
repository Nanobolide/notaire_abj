...
const selectUser = `SELECT u.id, u.etude_id, u.role, u.nom_affiche, u.fonction, u.niveau_acces,
        ...
        u.mfa_active, u.mfa_method, u.mfa_secret,
  ...
  try {
    ...
    if (isPg()) {
      try {
        ...
        user = rows[0];
        if (user?.id) {
          const { rows: mfaRows } = await db.query(
            `SELECT mfa_active, mfa_method, mfa_secret FROM utilisateurs WHERE id = $1`,
            [user.id]
          );
          user = { ...user, ...(mfaRows[0] || {}) };
        }
    ...
    const refus = async () => {
      await db.securityEvent(db, { typeEvenement: "auth.login_failed", severite: "warning", details: { identifiant } });
      return NextResponse.json({ erreur: "Identifiant ou mot de passe incorrect." }, { status: 401 });
    };
    if (!user) return refus();
    // Le Super-Admin plateforme n'est jamais bloqué par le statut d'une étude tenant.
    if (user.role !== "super_admin" && user.etude_statut !== "active")
    ...
    if (user.mfa_active) {
      const challengeToken = creerMfaChallenge(user);
      await db.securityEvent(db, {
        etudeId: user.etude_id,
        utilisateur: user.id,
        typeEvenement: "auth.mfa_challenge_created",
        details: { method: user.mfa_method || "totp" },
      });
      return NextResponse.json({ mfaRequired: true, method: user.mfa_method || "totp", challengeToken });
    }

    creerSession(user, { mfaLevel: "none" });
    await db.securityEvent(db, {
      etudeId: user.etude_id,
      utilisateur: user.id,
      typeEvenement: "auth.login_success",
      details: { mfa: false },
    });
