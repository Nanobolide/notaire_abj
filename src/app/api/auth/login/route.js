...
export async function POST(req) {
  ...
  return NextResponse.json({
    ...
    const codes = ["ECONNREFUSED", "ENOTFOUND", "28P01", "3D000"];
    const bd = codes.includes(e.code) || /connect|password|database/i.test(e.message);
    return NextResponse.json({
      erreur: bd
        ? "Base de données injoignable. Vérifiez le fichier .env (DATABASE_URL) et que PostgreSQL est démarré, puis relancez « npm run db:init » si la base n'existe pas."
