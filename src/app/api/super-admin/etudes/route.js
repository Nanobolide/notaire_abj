...
/** Liste des études avec leur forfait, statut et nombre de comptes. */
...
/** Changer le forfait ou le statut d'une étude. */
export async function PATCH(req) {
  try {
    ...
    if (forfait) await pool.query(`UPDATE etudes SET forfait = $1 WHERE id = $2`, [forfait, etudeId]);
    if (statut)  await pool.query(`UPDATE etudes SET statut  = $1 WHERE id = $2`, [statut, etudeId]);
