...
export async function PATCH(req, { params }) {
  ...
  try {
    ...
    // PRÉVISION (frais annoncés, versé) — le Clerc y a droit (proposition n°3).
    if (!saisitPrevision(s))
      for (const ch of ["valeur_acte","honoraires_totaux","montant_regle","statut_paiement"]) delete d[ch];
    // VENTILATION (émoluments, droits d'État, débours, autres) — Notaire et Comptable SEULS.
    if (!voitMontants(s))
    ...
    // C17 — Les MONTANTS de dépenses : Notaire et Comptable seulement. Le Formaliste,
    // même s'il ne voit plus la colonne, ne doit pas pouvoir en injecter par l'API.
    if (!saisitDepenses(s)) delete d.depenses_formalites;
    // Le STATUT des formalités : Formaliste, Notaire, Comptable. Jamais un Clerc.
    ...
    const ligne = await withTenant(s.etudeId, async (c) => {
      ...
      // C20 — Garde-fou financier après fusion des champs : le versé se compare au total
      // RÉELLEMENT facturé (ventilation du comptable), et à la prévision seulement à défaut.
