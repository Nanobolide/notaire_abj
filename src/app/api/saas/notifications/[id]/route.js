...
export async function PATCH(req, { params }) {
  try {
    ...
    if ("statut" in d && d.statut === "sent") sets.push(`envoye_le = ${nowSql()}`);
    vals.push(params.id);
    const { rows } = await query(`UPDATE saas_notifications SET ${sets.join(", ")} WHERE id = $${vals.length} RETURNING *`, vals);
    if (!rows[0]) return NextResponse.json({ erreur: "Notification introuvable." }, { status: 404 });
