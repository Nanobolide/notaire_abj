...
export async function PATCH(req, { params }) {
  try {
    ...
    const fields = ["quota_utilisateurs", "quota_stockage_go", "expire_le", "statut"];
    const sets = [];
    const vals = [];
    ...
    const { rows } = await query(
      `UPDATE saas_licenses SET ${sets.join(", ")}, modifie_le = ${nowSql()} WHERE id = $${vals.length} RETURNING *`,
      vals
    );
    if (!rows[0]) return NextResponse.json({ erreur: "Licence introuvable." }, { status: 404 });
