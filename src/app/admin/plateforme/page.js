...
export default function AdminPlateforme() {
  ...
  useEffect(() => { chargerEtudes(); chargerAnnonces(); chargerReglages(); }, []);
  ...
  const toggleEtude = (id) => setAnnonce((a) => ({
    ...a, etudes: a.etudes.includes(id) ? a.etudes.filter((x) => x !== id) : [...a.etudes, id],
  }));

  return (
    <>
      <h1>Forfaits, annonces & réglages</h1>
      <p className="sous-titre">Fonctions v3.5 : forfaits par étude, cloche d'annonces côté cabinet, service d'offres immobilières.</p>
            ...
            <tbody>
              {etudes.map((e) => (
                <tr key={e.id}>
                  ...
                  <td>{e.comptes}</td>
                  <td>
                    <select value={e.forfait || "essentiel"} onChange={(ev) => changerForfait(e.id, ev.target.value)} style={{ fontSize: 12 }}>
